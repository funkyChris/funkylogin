	...
	public void testClockService() {
		...
		try {
		    mockery.checking(new Expectations() {
                {
                    oneOf(membership).getProfilePathForConnectedIdentifier(); will(returnValue("/profiles/toto"));
                }
            });
			...
			// French format
			assertTrue(Pattern.matches("\\d\\d?\\s[^\\s]*\\s\\d{4}\\s\\d{2}:\\d{2} asked by /profiles/toto$", time));
			// US format
